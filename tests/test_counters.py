...
async def test_post_menu_and_zero_counts(server, test_client, cascades):
    data = {
        "title": f"My menu 1",
...
async def test_post_submenus_and_zero_dishes(server, test_client, cascades):
...
async def test_post_dishes(server, test_client, cascades):
    for menu_id in cascades:
...
async def test_menu_counts(server, test_client, cascades):
...
async def test_submenu_counts(server, test_client, cascades):
    for menu_id in cascades:
        for submenu_id in cascades[menu_id]:
...
async def test_delete_random_submenu(server, test_client, cascades):
    for menu_id in cascades:
...
async def test_menu_counts_after_submenu_delete(server, test_client, cascades):
    for menu_id in cascades:
        dishes_count = 0
        for submenu_id in cascades[menu_id]:
            dishes_count += len(cascades[menu_id][submenu_id])
...
async def test_random_dishes_delete(server, test_client, cascades):
    for menu_id in cascades:
        for submenu_id in cascades[menu_id]:
...
async def test_submenu_counts_after_random_dishes_delete(server, test_client, cascades):
    for menu_id in cascades:
        for submenu_id in cascades[menu_id]:
            response = await test_client.get(f"/menus/{menu_id}/submenus/{submenu_id}")
            response_json = response.json()
...
async def test_delete_menu(server, test_client, cascades):
    for menu_id in cascades:
