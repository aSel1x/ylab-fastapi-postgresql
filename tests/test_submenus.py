...
async def test_get_empty_submenus(server, test_client, store):
...
async def test_get_submenu_id(server, test_client, store):
    response = await test_client.get(f"/menus/{store['menu_id']}/submenus/{store['submenu']['id']}")
    assert response.status_code == 200
    assert response.json() == store['submenu']


async def test_get_submenus(server, test_client, store):
    response = await test_client.get(f"/menus/{store['menu_id']}/submenus")
    assert response.status_code == 200
    assert response.json() == [store["submenu"]]


async def test_patch_submenu(server, test_client, store):
    data = {
...
async def test_get_submenu_id_after_patch(server, test_client, store):
    response = await test_client.get(f"/menus/{store['menu_id']}/submenus/{store['submenu']['id']}")
    assert response.status_code == 200
    assert response.json() == store['submenu']


async def test_get_submenus_after_patch(server, test_client, store):
    response = await test_client.get(f"/menus/{store['menu_id']}/submenus")
    assert response.status_code == 200
    assert response.json() == [store["submenu"]]


async def test_delete_submenu(server, test_client, store):
    response = await test_client.delete(f"/menus/{store['menu_id']}/submenus/{store['submenu']['id']}")
    assert response.status_code == 200


async def test_get_submenu_id_after_delete(server, test_client, store):
    response = await test_client.get(f"/menus/{store['menu_id']}/submenus/{store['submenu']['id']}")
    assert response.status_code == 404


async def test_get_submenus_after_delete(server, test_client, store):
    response = await test_client.get(f"/menus/{store['menu_id']}/submenus")
    assert response.status_code == 200
    assert response.json() == []
